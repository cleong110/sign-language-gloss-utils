...
def text_to_glosses(text: str, gloss_vocabulary: set, uppercase=True):
    """Given some input text, extracts possible gloss matches, e.g. from asl-lex or ASL Citizen"""
    ...
    for word in text.split():
        ...
        for gloss in gloss_vocabulary:
            ...
            if gloss == word or re.fullmatch(rf"{re.escape(word)}_\d+", gloss):
...
def test_gloss_matching():
    ...
    test_tuples = [
        ...
        (
            # "OH I SEE", # I don't know if this occurs anywhere
        ...
        (
            "asllex:fry_",  # ASL Knowledge Graph
            "fry ",  # Note trailing space. ASL Lex 2.0
        ),
        (
            "murder ",  # note trailing space ASL Lex 2.0
            "asllex:murder_",  # ASL Knowledge graph
        ),
        (
            "asllex:advertise_2",  # ASL Knowledge graph has literally only got this one, no _1
            "advertise",  # ASL LEX has only got this, J_03_015
            "ADVERTISE",  # J_03_015 in ASL Citizen
            "advertise",  # Sem-lex metadata
        ),
        (
            "asllex:argue_2",  # the only "argue" in ASLKG
            "argue",  # the only "argue" in ASL LEX, F_03_072
            "ARGUE",  # F_03_072 in ASL Citizen
        ),
    ]
